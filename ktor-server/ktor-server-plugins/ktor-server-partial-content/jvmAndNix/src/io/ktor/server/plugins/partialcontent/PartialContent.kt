...
public val PartialContent: RouteScopedPlugin<PartialContentConfig> = createRouteScopedPlugin(
    ...
    onCall { call ->
        if (call.request.ranges() == null) {
            LOGGER.trace("Skip partial logging for ${call.request.uri}: no ranges specified")
            return@onCall
        }

        if (!call.isGetOrHead()) {
            LOGGER.trace("Skip partial logging for ${call.request.uri}: not a GET or HEAD request")
    ...
    on(BodyTransformedHook) { call, message ->
        ...
        if (rangeSpecifier == null) {
            LOGGER.trace("Add allowed range headers to ${call.request.uri}")
